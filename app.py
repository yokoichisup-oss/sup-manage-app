...
app.config["SECRET_KEY"] = os.environ.get(
    "SECRET_KEY", "a_default_secret_key_for_development"
)
if app.config["SECRET_KEY"] == "a_default_secret_key_for_development":
    logger.warning("SECRET_KEY がデフォルトのままです。本番環境では必ず環境変数で設定してください。")
...
def natural_sort_key(text: Optional[str]) -> List[object]:
    """
    自然順ソートのためのキー関数（例: B2 < B10 を正しく扱う）。
    Noneや空文字にも耐性あり。
    """
    t = text or ""
...
def get_selected_location(form) -> str:
    """
    ロケーション選択UI（プルダウン + その他入力）から値を確定する。
    """
...
@login_required
@member_required
def answer_attendance(attendance_id: int):
